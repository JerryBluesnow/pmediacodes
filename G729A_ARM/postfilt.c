 ...
 *---------------------------------------------------------------------------*/
  ...
  j = norm_l(temp);
  cmax = tkr_round(L_shl(cor_max, j));
  en = tkr_round(L_shl(ener, j));
  en0 = tkr_round(L_shl(ener0, j));
...
)
{
  ...
  exp = sub(norm_l(s), 1);
  gain_out = tkr_round(L_shl(s, exp));
